...
if selected_menu == "CH1: 개인과 사회의 가치와 웰빙":
    ...
    # Q1, 2, 3, 5, 6, 7 탭 생성
    tabs = st.tabs(["Q1", "Q2", "Q3", "Q5", "Q6", "Q7"])
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            q1_data = df.copy()
            q1_data['국가명'] = q1_data['국가'].map(code_to_country)
    ...
    with tabs[1]:
        ...
        with col1:
            q2_df = df.copy()
            q2_df["국가명"] = q2_df["국가"].map(code_to_country)
            fig, ax = plt.subplots(figsize=(12, 6))
            sns.violinplot(x="국가명", y="Q2", data=q2_df, scale="width", inner=None, cut=0, palette=color_palette, bw=0.35, ax=ax)
            ...
            for i, country in enumerate(country_order):
                ...
                if not c_data.empty:
                    ...
                    t1 = ax.text(i, mean_val, f'{mean_val:.1f}', color='red')
                    texts.append(t1)
            adjust_text(texts)
    ...
    with tabs[2]:
        ...
        with col1:
            q3_df = df.copy()
            q3_df["국가명"] = q3_df["국가"].map(code_to_country)
            fig, ax = plt.subplots(figsize=(12, 6))
            sns.violinplot(x="국가명", y="Q3", data=q3_df, scale="width", inner=None, cut=0, palette=color_palette, bw=0.35, ax=ax)
            # (Q2와 동일한 마커 로직 적용 가능)
            plt.xticks(rotation=45)
            st.pyplot(fig)
        with col2:
    ...
    with tabs[3]:
        ...
        col1, col2 = st.columns([3, 1])
        with col1:
            q5_labels = {"Q5_1_1":"가족", "Q5_1_2":"일/직업", "Q5_1_3":"물질적 풍요", "Q5_1_4":"가까운 관계", "Q5_1_5":"건강", "Q5_1_6":"자유", "Q5_1_7":"취미", "Q5_1_8":"배움/공부", "Q5_1_9":"연애", "Q5_1_10":"새로운 경험", "Q5_1_11":"신앙/믿음"}
            q5_cols = list(q5_labels.keys())
            q5_data = df.copy()
            q5_data[q5_cols] = q5_data[q5_cols].replace(99, np.nan)
            q5_data["국가명"] = q5_data["국가"].map(code_to_country)
            ...
            st.pyplot(fig)
        with col2:
            st.write(descriptions["Q5"])

    # --- Q6, Q7 탭 (동일한 가중치 로직 적용) ---
    for idx, q_num in enumerate(["Q6", "Q7"]):
        with tabs[idx+4]:
            st.subheader(f"{q_num} 분석 결과")
            col1, col2 = st.columns([3, 1])
            with col1:
                labels = {1:"개인의 자유", 2:"평등", 3:"가족", 4:"신에 대한 믿음", 5:"자연과 생명 보호", 6:"민주주의", 7:"자유시장경제", 8:"개인의 행복", 9:"사회적 약자 보호", 10:"법치와 질서", 11:"역사와 전통", 12:"공정함"}
                rank_cols = [f'{q_num}_1순위', f'{q_num}_2순위', f'{q_num}_3순위']
                weights = [3, 2, 1]
                
                scores = pd.DataFrame(0, index=country_order, columns=labels.values())
                temp_df = df.copy()
                temp_df["국가명"] = temp_df["국가"].map(code_to_country)
                
                for i, col in enumerate(rank_cols):
                    counts = temp_df.groupby("국가명")[col].value_counts(normalize=True).unstack().fillna(0)
